  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        const uploadTime = Math.min(3000, Math.max(1000, file.size / 100000));
      ...
      // Auto-close modal after all files are processed
      setTimeout(() => {
        ...
        setMessages(prev => [
          ...prev,
          {
            id: Date.now() + 2,
            type: 'bot',
            content: `🎉 Successfully processed ${newFiles.length} document${newFiles.length > 1 ? 's' : ''}! I'm now ready to answer questions based on your uploaded content.`,
            timestamp: new Date(),
            animated: true
          }
        ]);
